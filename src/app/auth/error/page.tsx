...
export default async function AuthErrorPage({ searchParams }: Props) {
  ...
  return (
    <div style={{ maxWidth: 440, margin: '60px auto', padding: '0 24px', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: '#1A1A1A', marginBottom: 16 }}>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 14, lineHeight: 1.7, color: '#1A1A1A' }}>{message}</p>
      </div>
      <p style={{ marginTop: 24, fontSize: 12 }}>
        <Link href="/auth/signin" style={{ color: '#534AB7', textDecoration: 'none' }}>← Torna alla pagina di accesso</Link>
      </p>
    </div>
  )
}
