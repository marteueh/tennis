...
export default async function AdminMatchEditPage({ params }: Props) {
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '32px 24px', fontFamily: "'DM Sans', sans-serif" }}>
      <nav style={{ marginBottom: 18, fontSize: 12, color: '#7A7870' }}>
        <Link href="/admin/matches" style={{ color: '#534AB7', textDecoration: 'none' }}>← Tutte le partite</Link>
        <span style={{ margin: '0 8px', color: 'rgba(26,26,26,0.2)' }}>·</span>
        <Link href={`/partite/${match.slug}`} target="_blank" style={{ color: '#7A7870', textDecoration: 'none' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: '#1A1A1A', lineHeight: 1.2 }}>
          {winner?.first_name} {winner?.last_name} <span style={{ fontWeight: 400, color: '#7A7870' }}>b.</span> {loser?.first_name} {loser?.last_name}
        </h1>
        <p style={{
          fontFamily: "'Playfair Display', Georgia, serif", fontSize: 18, color: '#534AB7',
        ...
        <form action={saveMatchEditorial} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#1A1A1A', cursor: 'pointer' }}>
            <input
              ...
              style={{ width: 16, height: 16, accentColor: '#C8A85C' }}
            />
            <span style={{ fontWeight: 500 }}>In primo piano (featured)</span>
            <span style={{ color: '#7A7870' }}>— viene mostrata in homepage e nella sezione Clerici</span>
          </label>
...
const labelStyle: React.CSSProperties = {
  ...
  textTransform: 'uppercase', color: '#7A7870', marginBottom: 4,
}
...
const primaryButton: React.CSSProperties = {
  ...
  padding: '9px 22px', background: '#534AB7', color: '#FFFFFF',
