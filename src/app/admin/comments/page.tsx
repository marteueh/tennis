...
const TABS: { value: 'pending' | 'flagged' | 'approved' | 'rejected'; label: string; color: string }[] = [
  { value: 'pending',  label: 'Da moderare', color: 'var(--accent)' },
...
export default async function AdminCommentsPage({ searchParams }: Props) {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px', fontFamily: "var(--font-sans)" }}>
      ...
      <form method="get" style={{ marginBottom: 18 }}>
        ...
        <input
          ...
          style={{
            ...
            border: '1px solid rgba(var(--ink-rgb),0.15)', borderRadius: 2, color: 'var(--ink)',
          }}
        />
      </form>
      ...
      {comments.length === 0 ? (
        <p style={{ fontSize: 13, color: 'rgba(var(--ink-rgb),0.4)', fontStyle: 'italic', padding: '32px 0', textAlign: 'center' }}>
...
function BulkActions() {
  return (
    <div style={{
      ...
      background: '#FFFFFF', border: '1px solid rgba(var(--ink-rgb),0.08)',
      borderRadius: 2, alignItems: 'center',
    }}>
      <span style={{ fontSize: 11, color: 'var(--muted)' }}>Azioni in massa sui selezionati:</span>
      <button
        type="submit"
  ...
  return (
    <div
      style={{
        ...
        borderLeft: c.status === 'flagged' ? '3px solid #DC2626'
                   : c.status === 'pending' ? '3px solid var(--accent)'
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
        <input type="checkbox" name="ids" value={c.id} style={{ marginTop: 4, accentColor: 'var(--accent)' }} />
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--muted)' }}>
            <Link href={`/partite/${c.match_slug}`} target="_blank" style={{ color: 'var(--accent)', textDecoration: 'none' }}>
...
function RoleBadge({ role }: { role: string }) {
  const cfg: Record<string, { bg: string; fg: string }> = {
    admin:   { bg: 'rgba(var(--gold-rgb),0.15)', fg: '#92400E' },
    trusted: { bg: 'rgba(34,197,94,0.1)',   fg: '#16A34A' },
    banned:  { bg: 'rgba(220,38,38,0.1)',   fg: '#991B1B' },
    user:    { bg: 'rgba(var(--ink-rgb),0.05)',   fg: 'var(--muted)' },
  }
...
const btnGhost: React.CSSProperties = {
  ...
  border: '1px solid rgba(var(--accent-rgb),0.3)', borderRadius: 2, cursor: 'pointer',
}
