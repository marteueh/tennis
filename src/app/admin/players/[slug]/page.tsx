  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '32px 24px', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ marginBottom: 18, fontSize: 12, color: '#7A7870' }}>
        <Link href="/admin/players" style={{ color: '#534AB7', textDecoration: 'none' }}>← Tutti i giocatori</Link>
        <span style={{ margin: '0 8px', color: 'rgba(26,26,26,0.2)' }}>·</span>
        <Link href={`/giocatori/${player.slug}`} target="_blank" style={{ color: '#7A7870', textDecoration: 'none' }}>
      ...
      <div style={{ marginBottom: 28, display: 'flex', alignItems: 'flex-start', gap: 20 }}>
        <div style={{ flex: 1 }}>
          ...
          <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 32, color: '#1A1A1A', lineHeight: 1.1 }}>
            {player.first_name} {player.last_name}
          </h1>
          <p style={{ fontSize: 12, color: '#7A7870', marginTop: 4 }}>
            {player.country_code} · #{player.atp_peak_rank ?? '–'} ATP · {player.grand_slams} Slam
          </p>
        </div>
        {player.photo_url && (
          <div style={{ width: 90, aspectRatio: '3/4', position: 'relative', border: '1px solid rgba(26,26,26,0.08)', flexShrink: 0 }}>
            <Image
      ...
      <Section title="Spot pubblicitari — aggiunta rapida YouTube">
        <p style={{ fontSize: 12, color: '#7A7870', marginBottom: 12, lineHeight: 1.6 }}>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              padding: '8px 18px', background: '#C8A85C', color: '#FFFFFF',
