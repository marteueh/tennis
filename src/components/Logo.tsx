...
export function Logo({ variant = 'dark', compact = false, className = '' }: LogoProps) {
  const textColor = variant === 'dark' ? '#ffffff' : '#1C1A17'
    ...
    <Link href="/" className={`inline-flex flex-col items-start select-none no-underline ${className}`}>
      ...
      <div className="flex items-center" style={{ gap: compact ? 7 : 10 }}>
        ...
        <svg
          ...
          aria-hidden="true"
        >
          <circle cx="26" cy="26" r="22" stroke="#9C7C3E" strokeWidth="2" />
          <path
      ...
      >
        <div style={{ flex: 1, height: 1, background: '#9C7C3E', opacity: 0.55, minWidth: 20 }} />
        <span
          style={{
            ...
            color: variant === 'dark' ? '#7C7568' : '#7C7568',
            whiteSpace: 'nowrap',
          }}
        >
